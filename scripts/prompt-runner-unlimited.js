 ...
 * Per-prompt LoRA OFF supported: --no-lora | prompt
...
function parsePromptLine(line) {
  ...
  var m = raw.match(/^(\d+)\s*[xX×]\s*(\d+)\s*\|\s*(.+)$/);
  if (m) {
  ...
  // Per-prompt LoRA control:
  //   --no-lora | prompt
  //   1024x576 | --no-lora | prompt
  var flag = raw.match(/^--(?:no-lora|lora-off|clear-lora)\s*\|\s*(.+)$/i);
  if (flag) {
    noLora = true;
    raw = flag[1].trim();
  }
...
function parsePrompts(raw) {
  return String(raw || "")
    ...
    .map(parsePromptLine)
    .filter(function (item) { return item.prompt.length > 0; });
}
...
try {
  ...
  for (var i = 0; i < promptList.length; i++) {
    for (var n = 0; n < imagesPerPrompt; n++) {
      ...
      var loraTag   = item.noLora ? "nolora_" : "";
      var promptTag = slugify(prompt) || "prompt";
      var filename  = prefix + "_" + modeTag + sizeTag + loraTag + promptTag + "_" + zeroPad(generatedCount, 4) + ".png";
